...
/*  0                   1                   2                   3
	...
	public int Version, Padding, Extension, CC,Marker, PayloadType, SequenceNumber, TimeStamp, Ssrc;
	//bits:			2		1		1		4	1			7			16 |			32		32
	...
	public RTPpacket(int PType, int Framenb, int Time, byte[] data, int data_length) {
		...
		Version = 2; Padding = 0; Extension = 0; CC = 0; Marker = 0; Ssrc = 0;

    	// fill changing header fields with params:
    	SequenceNumber = Framenb; TimeStamp = Time; PayloadType = PType;

    	// build the header bistream:
    	header = new byte[HEADER_SIZE]; setRtpHeader();

    	// fill the payload bitstream:
    	payload_size = data_length; payload = new byte[data_length];
	...
	public void setRtpHeader() {
    	header[0]	= (byte) (header[0] | Version << 6 | Padding << 5 | Extension << 4 | CC << 3);
    	header[1]	= (byte) ((PayloadType & 0xFF) | Marker << 7);
	...
	public void printheader() { printheader(HEADER_SIZE-4, header);  }

	//print the payload of a RTP packet @param n Number of bytes to print
	public void printpayload(int n) { printheader(n, payload); }
