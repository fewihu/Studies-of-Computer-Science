...
public class FecHandler {
  ...
  public ArrayList<RTPpacket> getNextRtpList() {
    ...
    for (int i = 1; i < rtpList.size(); i++) {
      RTPpacket p = getRtp(rtpList.get(i));
      //FIXME durch mich geändert:
      // - bezieht sich auf nächsten Kommentar
  ...
  private boolean checkCorrection(int nr) {
    ...
    System.out.println("Prüfe, ob ich verlorenes RTP Paket " + nr + " wiederherstellen kann");
    int numberLostPackets = 0;

    List<Integer> involvedRTPpackets = fecList.get(nr);
    if(involvedRTPpackets == null) return false;
    System.out.println("beteiligte RTP Pakete: " + involvedRTPpackets);

    int RTPseqNr;
    int j = 0;

    while(j < involvedRTPpackets.size()){
      RTPseqNr = involvedRTPpackets.get(j);

      if(rtpStack.get(RTPseqNr) == null) {
        numberLostPackets++;
        System.out.println("RTP Paket " + RTPseqNr + " liegt nicht vor");
      }
      j++;
    }

    if(fecStack.get(fecNr.get(nr)) == null) {
      System.out.println("??????????????????????????\nFECHandler: auch das FEC-Paket ist kaputt\nGruppenfehler" + fecGroupCounter);
      return false;
    }else {

      if (numberLostPackets < 2) {
        System.out.println("??????????????????????????\nFECHandler: " + numberLostPackets + " Pakete kaputt, kann alles retten");
        return true;
      } else {
        System.out.println("??????????????????????????\nFECHanlder: " + numberLostPackets + " Pakete kaputt, nichts zu retten\nGruppenfehler " + fecGroupCounter);
        return false;
      }
    }
  }
  ...
  private RTPpacket correctRtp(int nr) {
    ...
    List<Integer> involvedRTPpackets = fecList.get(nr);
    System.out.println("beteiligte RTP Pakete:\n" + involvedRTPpackets + " FEC Gruppengröße: " + fecGroupSize);

    for(int i = 0; i < involvedRTPpackets.size(); i++) {
        System.out.println("Hole RTP Paket zum Addieren");

        RTPpacket rtp = rtpStack.get(involvedRTPpackets.get(i));

        if(involvedRTPpackets.get(i) != nr) {
          fec.addRtp(rtp);
          System.out.println("Addiere folgendes RTP Paket: " + rtp.SequenceNumber);
        }
    }
  ...
  private void clearStack(int nr) {
    //TASK complete this method!
    fecList.remove(nr);
    int relatedFECpacket = fecNr.remove(nr);
    fecStack.remove(relatedFECpacket);
    rtpStack.remove(nr);
  }
