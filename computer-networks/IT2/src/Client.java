...
public class Client {
  ...
  /**
   * little function to set status-var, update statusLabel and print new status
   * @param status
   */
  private void setStatus(int status){
    ...
    String stMsg = "";
    switch (status){
      case 0: stMsg = "INIT";   break;
      case 1: stMsg = "READY";  break;
      case 2: stMsg = "PlAYING";break;
      default:  break;
    }
    System.out.println("new client state: " + stMsg);
    statusLabel.setText("Status: " + stMsg);
  }
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      if (state == INIT) {
        ...
        try {
          ...
          // open RTP-Socket + set Timeout to 5ms
    ...
    private void setStatistics() {
      ...
      pufferLabel.setText(
          "Puffer: "
              ...
              + "");
      statsLabel.setText(
          "Abspielzähler / verlorene Medienpakete // Bilder verloren: "
              + fec.getPlayCounter()
              + " / "
              + fec.getNrLost()
              + " // "
      ...
      fecLabel.setText(
          "FEC: korrigiert / nicht korrigiert: "
              ...
              + "  Ratio: "
              + ((double)fec.getNrCorrected() / ((double)fec.getNrNotCorrected() +1)));
    }
  }
  ...
  private void send_RTSP_request(String request_type) {
    ...
    switch(request_type){
      case "SETUP":
        rtspReq = "SETUP " + rtsp + "/trackID=0 RTSP/1.0" + CRLF +
                "CSeq: " + RTSPSeqNb + CRLF +
                //specify protocols + ports
                "Transport: RTP/UDP;unicast;client_port=" +
                RTP_RCV_PORT + "-" + (RTP_RCV_PORT+1) + CRLF;
        break;

      default:    //basically all the same
